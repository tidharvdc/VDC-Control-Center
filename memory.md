...
1. **התחברות וניהול סשנים:** מבוסס Supabase Auth עם שמירת מצב מאובטחת.
2. **ניהול מאובטח (Server Actions):** הקמת משתמשים חדשים על ידי מנהל מחלקה מתבצעת בשרת (`app/actions/adminUsers.ts`) באמצעות מפתח `SUPABASE_SERVICE_ROLE_KEY`.
3. **בקרת הרשאות (RLS):** מדיניות אבטחה המאפשרת למנהלי מחלקה ניהול מלא (CRUD) על טבלאות המשתמשים והפרויקטים, ולמהנדסים שליטה בדיווחים האישיים שלהם בלבד.
