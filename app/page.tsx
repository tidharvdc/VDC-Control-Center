...
import { 
  ...
  PenTool, Ruler, Filter, Printer, ArrowRight, FileText, Eye
} from 'lucide-react';
...
// --- ממשקים (Interfaces) למסד הנתונים ---
...
// --- מנועי תאריכים למחזורי דיווח (24 לחודש עד 23 לחודש הבא) ---
...
export default function Home() {
  ...
  // States - Costs Advanced Features
  ...
  // States - Meeting Management (Modal)
  ...
  // States לסינון בתוך עמודות הטבלה
  ...
  useEffect(() => {
    const checkUserAndFetchData = async () => {
      ...
      const { data: activeProjects, error: activeErr } = await supabase.from('projects').select('id, project_name, project_code, status, has_sub_stages, assigned_engineer, apartments_count, typologies_count, parent_typologies_count, sub_typologies_count').eq('status', 'פעיל').order('project_name');
      if (activeErr) {
        console.error("Projects Fetch Error:", activeErr);
        alert(`שגיאה בשליפת פרויקטים מהשרת: ${activeErr.message}\nאנא ודא שהעמודות קיימות ב-DB.`);
      } else if (activeProjects) {
        setActiveProjectsList(activeProjects);
      }

      const { data: allProjects } = await supabase.from('projects').select('id, project_name, project_code, status, has_sub_stages, assigned_engineer, apartments_count, typologies_count, parent_typologies_count, sub_typologies_count').order('project_name');
  ...
  const startImpersonation = (targetName: string) => {
    setImpersonatedEngineer(targetName);
    setCurrentUserRole('basic'); 
    setEngineerName(targetName);
    setFilterEngineer(targetName);
    setShowImpersonateMenu(false);
    setCurrentTab('reports');
    fetchReports(filterMonth, targetName, filterProject);
  };

  const stopImpersonation = () => {
    setImpersonatedEngineer(null);
    setCurrentUserRole(actualRole);
    setEngineerName(actualName);
    setFilterEngineer('');
    setCurrentTab('team');
    fetchReports(filterMonth, '', filterProject);
  };
  ...
  const handleTabChange = (tab: 'reports' | 'dashboard' | 'costs' | 'team') => { 
    const targetMonth = filterMonth || defaultActiveMonth;
    
    setIsCompareMode(false);
    setCostSelectedProjects([]);

    if (tab === 'costs' || tab === 'team') { 
      setFilterEngineer(''); 
      setFilterProject(''); 
  ...
  // --- ניהול פגישות עבודה במודל מרכזי חכם (Central Modal) ---
  ...
  const selectProjectForMeeting = async (projName: string) => {
    ...
    // 1. שליפת היסטוריית פגישות
    const { data: meetings } = await supabase
      .from('work_meetings')
      .select('*')
      .eq('engineer_name', drawerEngineer)
      .eq('project_name', projName)
      .order('meeting_date', { ascending: false });
      
    if (meetings) setMeetingHistory(meetings);
    
    // 2. שליפת דיווחים מהשבוע החולף להקשר מהיר
  ...
  const handleDeleteMeeting = async (id: number) => {
    ...
    if (!error) {
      ...
      alert('שגיאה במחיקת פגישה: ' + error.message);
    }
  };
  ...
  const handleMeetingSubmit = async (e: React.FormEvent) => {
    ...
    if (editingMeeting) {
      const { error } = await supabase.from('work_meetings').update({
        ...
        progress_status: meetingProgress,
        bottlenecks: meetingBottlenecks,
        weekly_focus: meetingFocus,
        modelers_tracking: meetingModelers
      }).eq('id', editingMeeting.id);
      
      setMeetingLoading(false);
      
      if (error) {
        alert('שגיאה בעדכון סיכום הפגישה: ' + error.message);
      } else {
        setIsNewMeetingOpen(false);
        setEditingMeeting(null);
        selectProjectForMeeting(drawerProject);
      }
    } else {
      const { error } = await supabase.from('work_meetings').insert([{
        meeting_date: meetingDate,
        manager_name: actualName, 
        engineer_name: drawerEngineer,
        project_name: drawerProject,
        progress_status: meetingProgress,
        bottlenecks: meetingBottlenecks,
        weekly_focus: meetingFocus,
        modelers_tracking: meetingModelers
      ...
      if (error) {
        alert('שגיאה בשמירת סיכום הפגישה: ' + error.message);
      } else {
        setIsNewMeetingOpen(false);
        selectProjectForMeeting(drawerProject); 
      }
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    ...
    const { data: dayReports, error: checkError } = await supabase
      .from('work_reports')
      .select('scope')
    ...
    if (checkError) {
      ...
      return alert('שגיאה בבדיקת מגבלת הדיווח היומית.');
    }
    ...
    if (currentDayTotal + requestedScopeVal > 1.0) {
      ...
      return alert(`חריגה ממגבלת הדיווח היומית לאותו יום!\nכבר דווח עבור יום זה: ${reportedText}.\nלא ניתן לדווח משרה של '${scope}' בנוסף.`);
    }
    ...
    if (!error) { 
      ...
      const fetchTarget = filterMonth || defaultActiveMonth;
      fetchReports(fetchTarget, filterEngineer, filterProject); 
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    const { data: dayReports, error: checkError } = await supabase
      .from('work_reports')
      .select('scope, id')
      .eq('report_date', editDate)
      .eq('engineer_name', editEngineerName)
      .neq('id', editingReport.id); 
    ...
    if (currentDayTotal + requestedScopeVal > 1.0) {
      setEditLoading(false);
      const reportedText = currentDayTotal === 0.5 ? 'חצי יום (50%)' : currentDayTotal === 1.0 ? 'יום מלא (100%)' : `${currentDayTotal * 100}%`;
      return alert(`חריגה ממגבלת הדיווח היומית לאותו יום!\nכבר דווח עבור יום זה: ${reportedText} (לא כולל רשומה זו).\nלא ניתן לעדכן למשרה של '${editScope}'.`);
    }
  ...
  const handleAdminUserSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) { alert(`המשתמש הוקם בהצלחה!`); setNewUserEmail(''); setNewUserFullName(''); } 
  ...
  const getMissingDates = (engReports: WorkReport[], filterMonthStr: string) => {
    ...
    while (d <= checkEnd) {
        ...
        if (dayOfWeek >= 0 && dayOfWeek <= 4) { // ראשון (0) עד חמישי (4)
            ...
            if (scope < 1) { 
                missingDates.push({ date: dString, missing: 1 - scope });
            }
        }
  ...
  const generateDashboardData = () => {
    ...
    const relevantEngineers = currentUserRole === 'manager' 
        ? [engineerName, ...orgUsers.filter(u => u.manager_name === engineerName).map(u => u.full_name)]
        : activeEngineers;

    relevantEngineers.forEach(eng => {
        data[eng] = { total: 0, projects: {}, reports: [] };
    });
    ...
    reports.forEach(report => {
      ...
      if (!data[engName].projects[projName]) {
          data[engName].projects[projName] = { totalDays: 0, stages: {} };
      }
      data[engName].projects[projName].totalDays += days;
      
      if (!data[engName].projects[projName].stages[stageName]) {
          data[engName].projects[projName].stages[stageName] = { days: 0, subStages: {} };
      }
      data[engName].projects[projName].stages[stageName].days += days;

      if (subStageName) {
          if (!data[engName].projects[projName].stages[stageName].subStages[subStageName]) {
              data[engName].projects[projName].stages[stageName].subStages[subStageName] = 0;
          }
    ...
    return relevantEngineers.map(engName => {
      ...
      const projects = projKeys.map((pName, index) => {
        ...
        const stagesArray = Object.keys(pData.stages).map(sName => {
           ...
           const subArray = Object.keys(sData.subStages).map(subName => ({
               name: subName, days: sData.subStages[subName]
        ...
        return { 
          name: pName, 
          days: pData.totalDays, 
          percentage: engData.total > 0 ? Math.round((pData.totalDays / engData.total) * 100) : 0, 
          color: colors[index % colors.length],
          stages: stagesArray,
          stats: {
            has_sub_stages: dbProject?.has_sub_stages,
            apartments_count: dbProject?.apartments_count,
            typologies_count: dbProject?.typologies_count,
            parent_typologies_count: dbProject?.parent_typologies_count,
            sub_typologies_count: dbProject?.sub_typologies_count
          }
        };
      }).sort((a, b) => b.days - a.days);
    ...
    .filter(eng => !(eng.isManagerLevel && eng.total_days === 0)) 
    .sort((a, b) => a.engineer_name.localeCompare(b.engineer_name));
  };
  ...
  const generateCostData = () => {
    ...
    allProjectsList.forEach(p => {
        ...
        if (!globalProjectCosts[displayName]) {
            globalProjectCosts[displayName] = { 
              ...
              stagesMap: {},
              projectStats: {
                has_sub_stages: p.has_sub_stages,
                apartments_count: p.apartments_count,
                typologies_count: p.typologies_count,
                parent_typologies_count: p.parent_typologies_count,
                sub_typologies_count: p.sub_typologies_count
              }
            };
        }
    });
    ...
    Object.keys(reportsByMonth).forEach(month => {
      ...
      mReports.forEach(r => {
         ...
         if (cleanProjName === 'אחר (פירוט בהערות)') {
            engData[engName].otherDays += days;
         } else {
            ...
            if (!engData[engName].projects[cleanProjName].stages[stageKey]) {
                engData[engName].projects[cleanProjName].stages[stageKey] = { days: 0, stageName, subStageName };
            }
      ...
      Object.keys(engData).forEach(eng => {
         ...
         activeProjects.forEach(proj => {
            ...
            Object.keys(projData.stages).forEach(stageKey => {
                ...
                if (subStageName) {
                    if (!gProj.stagesMap[stageName].subStages[subStageName]) {
                        gProj.stagesMap[stageName].subStages[subStageName] = { cost: 0, days: 0 };
                    }
    ...
    const processedData = Object.keys(globalProjectCosts).map(p => {
       ...
       const stagesArray = Object.keys(pData.stagesMap).map(s => {
           const subArray = Object.keys(pData.stagesMap[s].subStages).map(sub => ({
               name: sub, cost: pData.stagesMap[s].subStages[sub].cost, days: pData.stagesMap[s].subStages[sub].days
       ...
       return { 
         ...
         baseDays: pData.baseDays, 
         engineers: engineersArray, 
         stages: stagesArray,
         stats: pData.projectStats 
       };
    }).sort((a,b) => b.totalCost - a.totalCost);

    const finalCostData = processedData.filter(p => 
        p.totalCost > 0 && 
        p.name !== 'אחר (פירוט בהערות)' && 
        !p.name.includes('תקורות חברה')
    );
  ...
  const displayedCostData = costSelectedProjects.length > 0 
    ? costData.filter(p => costSelectedProjects.includes(p.name)) 
    : costData;
  ...
  // =========================================================================
  // לוגיקה דינמית לניהול מפת כוח אדם (Org Chart)
  // =========================================================================
  
  const getProjectsForEngineer = (engName: string) => {
    return allProjectsList
      .filter(p => p.assigned_engineer && p.assigned_engineer.includes(engName))
      .map(p => getProjectDisplayName(p, p.project_name));
  };

  const pipelineProjects = allProjectsList
    .filter(p => safeString(p.status) === 'עתידי')
    .map(p => getProjectDisplayName(p, p.project_name));

  const rootManager = orgUsers.find(u => u.full_name === actualName) || { 
    full_name: actualName, 
    role: actualRole === 'manager' ? 'מנהל VDC' : 'מנהל מחלקת VDC' 
  };
  ...
  const OrgCard = ({ user, projects, colorData, displayRole }: { user: any, projects: string[], colorData: any, displayRole: string }) => {
    ...
    return (
      <div 
        onClick={() => openEngineerDrawer(user.full_name)}
        className={`relative p-4 rounded-xl border border-slate-700 bg-slate-800/90 shadow-lg flex flex-col items-center text-center w-[230px] transition-all cursor-pointer hover:shadow-2xl hover:border-slate-400 hover:scale-105 z-10`}
      >
        ...
        <div className="text-[11px] font-bold text-slate-300 whitespace-nowrap uppercase tracking-widest mt-1.5 mb-4 bg-slate-900/60 px-3 py-0.5 rounded shadow-inner border border-slate-700/50">
          {displayRole}
        </div>
  ...
  return (
    <React.Fragment>
      ...
      {/* 
        ========================================================
        מנגנון הדפסה ל-PDF (נסתר על המסך, מוצג רק בהדפסה)
        ========================================================
      */}
      <div className="hidden print:block text-black bg-white font-sans w-full" dir="rtl">
        ...
        <div className="text-center mb-6 border-b border-slate-800 pb-3">
          ...
          <h2 className="text-sm font-medium text-slate-600">
            תקופת דיווח: {filterMonth} <span> {displayDateRange}</span>
          </h2>
        </div>

        <div className="flex justify-between border-b-2 border-black pb-1 mb-3 text-xs font-bold px-2">
           <span className="w-16">אחוז משרה</span>
           <span className="flex-1 text-right">פרויקט</span>
        </div>
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 text-right" style={{ columnRule: '1px solid #e2e8f0' }}>
          {dashboardData.map((engineer, idx) => (
             <div key={idx} className="break-inside-avoid mb-6 page-break-inside-avoid shadow-sm border border-slate-200 rounded overflow-hidden">
                <div className="bg-slate-100 font-bold text-[11px] p-2 border-b border-slate-300 text-slate-800 flex justify-between">
                   <span>{engineer.engineer_name}</span>
                   <span>סה"כ ימים: {engineer.total_days}</span>
                </div>
                   ...
                   {engineer.projects.map((proj: any, pIdx: number) => (
                     <div key={pIdx} className="flex border-b border-slate-100 last:border-0 p-2">
                        ...
                        <div className="flex-1 flex flex-col gap-0.5">
                           ...
                           {/* תצוגת נתוני פרויקט ב-PDF - לוגיקה חכמה */}
                           {(() => {
                              const s = proj.stats || {};
                              const aptVal = s.apartments_count;
                              const typVal = s.typologies_count;
                              const pTypVal = s.parent_typologies_count;
                              const sTypVal = s.sub_typologies_count;

                              const hasApt = aptVal !== null && aptVal !== undefined && String(aptVal).trim() !== '';
                              const hasTyp = typVal !== null && typVal !== undefined && String(typVal).trim() !== '';
                              const hasPTyp = pTypVal !== null && pTypVal !== undefined && String(pTypVal).trim() !== '';
                              const hasSTyp = sTypVal !== null && sTypVal !== undefined && String(sTypVal).trim() !== '';
                              
                              const hasAny = hasApt || hasTyp || hasPTyp || hasSTyp;
                              if (!hasAny) return null;

                              return (
                                <div className="flex flex-wrap gap-x-2 gap-y-0.5 text-[9px] text-slate-500 font-medium mt-0.5">
                                    {hasApt && <span>דירות: {aptVal}</span>}
                                    {s.has_sub_stages ? (
                                        <>
                                            {hasPTyp && <span>טיפוסי אב: {pTypVal}</span>}
                                            {hasSTyp && <span>תתי-טיפוס: {sTypVal}</span>}
                                        </>
                                    ) : (
                                        hasTyp && <span>טיפוסים: {typVal}</span>
                                    )}
                                </div>
                              );
                           })()}
        ...
        <div className="mt-8 text-center text-[9px] text-slate-400 font-mono pt-4 border-t border-slate-200">
          הופק באמצעות VDC Control Center • תאריך הפקה: {new Date().toLocaleString('he-IL')}
        </div>
      </div>

      {/* 
        ========================================================
        האפליקציה הראשית (מוצגת על המסך, נסתרת בהדפסה)
        ========================================================
      */}
      <div 
        className={`print:hidden min-h-screen ${currentTab === 'team' ? 'bg-slate-950 text-slate-300' : 'bg-slate-50 text-slate-800'} pb-12 overflow-x-hidden ${heebo.className} transition-colors duration-500`} 
        dir="rtl"
        style={{ backgroundImage: currentTab === 'team' ? 'none' : 'radial-gradient(#cbd5e1 1px, transparent 0)', backgroundSize: '24px 24px' }}
      >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5 flex items-center gap-1.5">
                  <HardHat className="w-4 h-4" /> מחובר: <span className="font-semibold text-slate-200">{engineerName}</span> 
                  <span className="opacity-50">|</span> 
                  {currentUserRole === 'department_manager' ? 'ניהול מחלקה' : currentUserRole === 'manager' ? 'ניהול פרויקט' : 'הנדסה'}
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3 items-center">
              ...
              {currentUserRole === 'department_manager' && (
                <button 
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-200 rounded-md font-medium hover:bg-slate-700 hover:text-white transition border border-slate-700 shadow-sm text-sm"
                >
                  <Users className="w-4 h-4" /> צוות
                </button>
              )}
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-500 transition shadow-md text-sm"
              >
        ...
        <div className={`${currentTab === 'team' ? 'max-w-[100%] 2xl:max-w-[95%]' : 'max-w-7xl'} mx-auto space-y-6 px-6 transition-all duration-500`}>
          ...
          {/* Navigation Tabs - גלויות למנהלים */}
          {(currentUserRole === 'department_manager' || currentUserRole === 'manager') && (
            <div className={`flex border-b overflow-x-auto gap-8 mb-6 ${currentTab === 'team' ? 'border-slate-800' : 'border-slate-300'}`}>
              <button onClick={() => handleTabChange('reports')} className={`pb-3 flex items-center gap-2 text-sm font-bold border-b-2 whitespace-nowrap transition ${currentTab === 'reports' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-400'}`}>
                <List className="w-4 h-4" /> טבלת דיווחים
              </button>
              <button onClick={() => handleTabChange('dashboard')} className={`pb-3 flex items-center gap-2 text-sm font-bold border-b-2 whitespace-nowrap transition ${currentTab === 'dashboard' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-400'}`}>
                <BarChart3 className="w-4 h-4" /> דאשבורד העמסות
              </button>
              {currentUserRole === 'department_manager' && (
                <button onClick={() => handleTabChange('costs')} className={`pb-3 flex items-center gap-2 text-sm font-bold border-b-2 whitespace-nowrap transition ${currentTab === 'costs' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-400'}`}>
                  <Wallet className="w-4 h-4" /> תמחור ובקרת תקציב
                </button>
              )}
              <button onClick={() => handleTabChange('team')} className={`pb-3 flex items-center gap-2 text-sm font-bold border-b-2 whitespace-nowrap transition ${currentTab === 'team' ? 'border-blue-600 text-blue-500' : 'border-transparent text-slate-500 hover:text-slate-400'}`}>
                <Network className="w-4 h-4" /> מפת כוח אדם
              </button>
            </div>
          )}

          {/* FORMS & FILTERS */}
          {currentTab === 'reports' && showAdminForm && currentUserRole === 'department_manager' && (
             <form onSubmit={handleAdminUserSubmit} className="bg-white p-6 rounded-md shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-3 gap-5 text-slate-800">
               <div className="md:col-span-3 border-b border-slate-100 pb-3 mb-1"><h2 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Users className="w-5 h-5 text-blue-600" /> הוספת מהנדס למערכת</h2></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">שם מלא</label><input type="text" required value={newUserFullName} onChange={(e) => setNewUserFullName(e.target.value)} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition" /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">אימייל ארגוני</label><input type="email" required value={newUserEmail} onChange={(e) => setNewUserEmail(e.target.value)} className="w-full text-left rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition" dir="ltr" /></div>
              ...
              <div className="md:col-span-3 flex justify-end pt-2"><button type="submit" disabled={adminFormLoading} className="px-6 py-2.5 bg-slate-800 text-white rounded-md font-medium hover:bg-slate-700 transition shadow-sm text-sm disabled:bg-slate-300">הקמת משתמש</button></div>
            </form>
          )}

          {currentTab === 'reports' && showReportForm && (
             <form onSubmit={handleReportSubmit} className="bg-white p-6 rounded-md shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-3 gap-5 transition-all text-slate-800">
              ...
              <div className={`${!projectName ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <select required value={stage} onChange={(e) => {
                 ...
                 <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                   ...
                   <select required value={subStage} onChange={(e) => setSubStage(e.target.value)} className="w-full rounded-md border-blue-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition bg-blue-50/30 text-blue-900">
                     ...
                     {availableSubStages.map((s) => (
                       <option key={s.id} value={s.sub_stage_name}>{s.sub_stage_name}</option>
                     ))}
                   </select>
                 </div>
              )}
          ...
          {(currentTab === 'reports' || currentTab === 'dashboard' || (currentTab === 'costs' && costSubTab === 'monthly' && !isCompareMode)) && (
            <form onSubmit={handleFilterSubmit} className="bg-white p-4 rounded-md shadow-sm border border-slate-200 flex flex-col md:flex-row gap-4 items-end text-slate-800">
                  ...
                  <div className="flex-1 w-full">
                    ...
                    <datalist id="projects-list">
                      {allProjectsList.map(proj => (
                        <option key={proj.id} value={getProjectDisplayName(proj, proj.project_name)} />
                      ))}
                    </datalist>
                  </div>
                </>
          ...
          {currentTab === 'team' && (currentUserRole === 'department_manager' || currentUserRole === 'manager') && (
            <div className={`flex flex-col xl:flex-row items-start gap-8 mt-2 animate-in fade-in duration-500 w-full ${currentUserRole === 'manager' ? 'justify-center' : ''}`}>
              ...
              {/* Pipeline Sidebar - מוצג למנהל מחלקה בלבד */}
              {currentUserRole === 'department_manager' && (
              ...
              {/* Main Org Chart Area (Dark Blueprint, Scrollable) */}
              <div 
                ...
                <div className="min-w-max flex flex-col items-center pb-12">
                  ...
                  <div className="relative w-full mt-0">
                    ...
                    <div className="flex justify-center items-start pt-8 gap-12 px-8 min-w-max">
                      ...
                      {/* Dynamic Team Leader Pillars - Only visible to Department Manager */}
                      ...
                      <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider font-bold">
                        ...
                        <th className="p-4 text-slate-700 relative">
                          <div className="flex items-center gap-1.5 cursor-pointer hover:text-blue-600 transition" onClick={() => setShowEngMenu(!showEngMenu)}>
                            <HardHat className="w-4 h-4" /> מהנדס 
                            <Filter className={`w-3.5 h-3.5 ${colFilterEngineers !== null ? 'text-blue-600 fill-blue-100' : 'text-slate-400'}`} />
                          </div>
                          {showEngMenu && (
                            <>
                              ...
                              <div className="absolute top-12 right-4 w-56 bg-white border border-slate-200 shadow-xl rounded-md z-50 p-3 max-h-72 overflow-y-auto text-sm font-normal">
                                ...
                                <div className="pt-2 mt-2 border-t flex justify-between gap-2">
                                  <button onClick={() => setColFilterEngineers(null)} className="text-xs font-bold text-blue-600 hover:underline">בחר הכל</button>
                                  <button onClick={() => setColFilterEngineers([])} className="text-xs text-slate-500 hover:underline">נקה הכל</button>
                                </div>
                              </div>
                            </>
                          )}
                        ...
                        <th className="p-4 text-slate-700 relative">
                          <div className="flex items-center gap-1.5 cursor-pointer hover:text-blue-600 transition" onClick={() => setShowProjMenu(!showProjMenu)}>
                            <Building2 className="w-4 h-4" /> פרויקט מדווח
                            <Filter className={`w-3.5 h-3.5 ${colFilterProjects !== null ? 'text-blue-600 fill-blue-100' : 'text-slate-400'}`} />
                          </div>
                          {showProjMenu && (
                            <>
                              ...
                              <div className="absolute top-12 right-4 w-64 bg-white border border-slate-200 shadow-xl rounded-md z-50 p-3 max-h-72 overflow-y-auto text-sm font-normal">
                                ...
                                <div className="pt-2 mt-2 border-t flex justify-between gap-2">
                                  <button onClick={() => setColFilterProjects(null)} className="text-xs font-bold text-blue-600 hover:underline">בחר הכל</button>
                                  <button onClick={() => setColFilterProjects([])} className="text-xs text-slate-500 hover:underline">נקה הכל</button>
                                </div>
                              </div>
                            </>
                          )}
                        ...
                        <tr key={report.id} className="hover:bg-blue-50/30 transition">
                          ...
                          <td className="p-4">
                            <div className="flex flex-col gap-1.5">
                              ...
                              {report.sub_stage && (
                                <span className="text-[11px] text-slate-500 pr-1 truncate font-medium">↳ {report.sub_stage}</span>
                              )}
                            </div>
                          </td>

                          <td className="p-4 text-slate-600 text-[13px] max-w-[200px] break-words whitespace-normal">
                             {report.notes ? report.notes : <span className="text-slate-300">-</span>}
                          </td>
                          ...
                          {(currentUserRole === 'manager' || currentUserRole === 'department_manager') && (
                            <td className="p-4">
                              <div className="flex items-center justify-center gap-2">
                                <button onClick={() => openEditModal(report)} className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white text-slate-600 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 rounded border border-slate-200 font-medium transition-all shadow-sm text-xs">
                                  <Edit2 className="w-3.5 h-3.5" /> עריכה
                                </button>
                                <button onClick={() => handleDelete(report.id)} className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white text-slate-600 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 rounded border border-slate-200 font-medium transition-all shadow-sm text-xs">
          ...
          {/* --- DASHBOARD TAB (העמסות ודיווחים חסרים) --- */}
          {currentTab === 'dashboard' && (currentUserRole === 'department_manager' || currentUserRole === 'manager') && (
            <>
              {currentUserRole === 'department_manager' && (
                <div className="flex justify-end mb-4">
                  <button onClick={() => window.print()} className="flex items-center gap-2 bg-slate-800 text-white px-5 py-2.5 rounded-md font-bold hover:bg-slate-700 transition shadow-md">
                    <Printer className="w-5 h-5" /> הפק דו"ח PDF
                  </button>
                </div>
              )}
                ...
                {dashboardData.map((engineer, idx) => (
                  <div key={idx} className="bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                    <div className="p-5 border-b border-slate-100 flex justify-between items-start md:items-center bg-slate-50/50 relative">
                      ...
                      {/* כפתור דיווחים חסרים שמפעיל את חלון המודל (Modal) */}
                      {engineer.missingDates && engineer.missingDates.length > 0 && (
                        <div className="mt-1 md:mt-0">
                          <button 
                            onClick={() => setOpenMissingEng(engineer.engineer_name)}
                            className="flex items-center gap-1.5 bg-rose-50 text-rose-600 px-3 py-1.5 rounded-md text-xs font-bold border border-rose-200 hover:bg-rose-100 transition shadow-sm"
                          >
                            <Search className="w-3.5 h-3.5" />
                            חסרים {engineer.missingDates.length} דיווחים
                          </button>
                        </div>
                      )}
                    </div>
                            ...
                            return (
                              <React.Fragment key={pIdx}>
                                <tr 
                                  ...
                                  onClick={() => toggleDashExpand(projectKey)}
                                >
                                  ...
                                  <td className="py-3 text-right flex items-center justify-end gap-2 truncate max-w-[160px]" title={proj.name}>
                                    <span className="truncate group-hover:text-blue-600 transition-colors">{proj.name}</span>
                                    <span className={`w-2.5 h-2.5 rounded-sm flex-shrink-0 ${proj.color}`}></span>
                                    <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isExpanded ? 'rotate-180 text-blue-500' : ''}`} />
                                  </td>
                                </tr>
                                ...
                                {isExpanded && proj.stages.map((stg: any, sIdx: number) => (
                                  <React.Fragment key={`stg-${sIdx}`}>
                                    <tr className="bg-slate-50/80 border-t border-slate-100">
                                      ...
                                      <td className="py-2 text-center text-slate-500 text-xs font-semibold">{stg.days} ימ׳</td>
                                      <td className="py-2 text-right pr-4 text-xs text-slate-600 font-semibold flex justify-end gap-1.5 items-center">
                                        {stg.name} <Layers className="w-3.5 h-3.5 text-slate-400" />
                                      </td>
                                    </tr>
                                    {stg.subStages.map((sub: any, subIdx: number) => (
                                      <tr key={`sub-${subIdx}`} className="bg-slate-100/50">
                                        <td className="py-1.5"></td>
                                        <td className="py-1.5 text-center text-slate-400 text-[11px] font-medium">{sub.days} ימ׳</td>
                                        <td className="py-1.5 text-right pr-8 text-[11px] text-slate-500 font-medium">↳ {sub.name}</td>
                                      </tr>
                                    ))}
                                  </React.Fragment>
                                ))}
          ...
          {/* מודל (Modal) להצגת הדיווחים החסרים במרכז המסך */}
              ...
              <div className="bg-white rounded-md shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 text-slate-800 animate-in zoom-in-95 duration-200">
                <div className="bg-slate-800 p-4 border-b border-slate-700 flex justify-between items-center">
                  <h2 className="text-lg font-bold text-white flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-rose-400" /> דיווחים חסרים - {openMissingEng}
                  </h2>
                  <button onClick={() => setOpenMissingEng(null)} className="text-slate-400 hover:text-white transition"><X className="w-5 h-5" /></button>
                </div>
                      ...
                      <div className="flex flex-col gap-2.5 text-sm text-slate-700">
                        ...
                        {engData.missingDates.map((md: any, i: number) => (
                          <div key={i} className="flex justify-between items-center p-3 bg-slate-50 hover:bg-slate-100 rounded-md border border-slate-100 transition-colors">
                             ...
                             <span className={`text-xs px-2.5 py-1 rounded font-bold shadow-sm ${md.missing === 0.5 ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-rose-100 text-rose-700 border border-rose-200'}`}>
                               {md.missing === 0.5 ? 'חסר חצי יום' : 'חסר יום מלא'}
                             </span>
                          </div>
                        ))}
                      </div>
                ...
                <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-end">
                  <button onClick={() => setOpenMissingEng(null)} className="px-5 py-2.5 bg-slate-200 text-slate-700 rounded-md font-medium hover:bg-slate-300 transition text-sm shadow-sm">סגור חלון</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 text-slate-800">
              ...
              {!isCompareMode && (
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        <>
                          ...
                          <div className="absolute top-12 right-0 w-64 bg-white border border-slate-200 shadow-xl rounded-md z-50 p-3 max-h-72 overflow-y-auto text-sm font-normal">
                             ...
                             <div className="pt-2 mt-2 border-t flex justify-end">
                               <button onClick={() => setCostSelectedProjects([])} className="text-xs font-bold text-blue-600 hover:underline">נקה סינון (הצג הכל)</button>
                             </div>
                          </div>
                        </>
                      )}
                  ...
                  {costSubTab !== 'monthly' && (
                    <button onClick={() => { setIsCompareMode(true); setCompareSelected([]); }} className="flex items-center justify-center w-full md:w-auto gap-2 px-5 py-2 bg-slate-800 text-white rounded-md font-bold hover:bg-slate-700 transition shadow-md text-sm">
                       <BarChart3 className="w-4 h-4"/> השוואת פרויקטים
                    </button>
                  )}
                </div>
              )}
                           ...
                           <tbody className="divide-y divide-slate-100">
                                 ...
                                 <>
                                    ...
                                    <tr>
                                       ...
                                       {compareSelected.map(pName => {
                                          ...
                                          return <td key={pName} className="p-3 font-black text-emerald-700 border-r border-slate-100 bg-emerald-50/30">
                                             {vdcCost !== null ? `₪ ${Math.round(vdcCost).toLocaleString()}` : '-'}
                                          </td>
                                       })}
                                    </tr>
                                    <tr>
                                       ...
                                       {compareSelected.map(pName => {
                                          const p = displayedCostData.find(x => x.name === pName);
                                          let aptCost = null;
                                          if (p?.stats?.apartments_count > 0) {
                                             aptCost = p.totalCost / p.stats.apartments_count;
                                          }
                                          return <td key={pName} className="p-3 font-black text-purple-700 border-r border-slate-100 bg-purple-50/30">
                                             {aptCost !== null ? `₪ ${Math.round(aptCost).toLocaleString()}` : '-'}
                                          </td>
                                       })}
                                    </tr>
                                 </>
                              ...
                              {Array.from(new Set(compareSelected.flatMap(pName => displayedCostData.find(x => x.name === pName)?.stages.map((s:any) => s.name) || []))).map(stageName => (
                                 <tr key={stageName} className="hover:bg-slate-50 transition-colors">
                                    <td className="p-3 font-bold text-slate-600 bg-slate-50/30 border-l border-slate-200">
                                      <div className="flex items-center gap-1.5"><Layers className="w-3.5 h-3.5 text-slate-400"/> {stageName}</div>
                                    </td>
                                    {compareSelected.map(pName => {
                                       const p = displayedCostData.find(x => x.name === pName);
                                          ...
                                          <td key={pName} className="p-3 border-r border-slate-100">
                                             {s ? (
                                                <div className="flex flex-col">
                                                   <span className="font-bold text-slate-800">₪ {Math.round(s.cost).toLocaleString()}</span>
                                                   <span className="text-[11px] text-slate-400 font-medium">{s.days} ימי עבודה</span>
                                                </div>
                ...
                <>
                  ...
                  {(() => {
                    ...
                    const renderProjectCard = (project: any, idx: number) => {
                      ...
                      let vdcCostPerTypology = null;
                      let vdcCostPerParentTypology = null;
                      let vdcCostPerSubTypology = null;
                      let costPerApartment = null;
                      
                      if (costSubTab !== 'monthly') {
                         if (project.stats?.apartments_count > 0) {
                            costPerApartment = project.totalCost / project.stats.apartments_count;
                         }
                         ...
                         if (!project.stats?.has_sub_stages && project.stats?.typologies_count > 0) {
                           ...
                           const standardFixesCost = standardFixesStage ? standardFixesStage.cost : 0;
                      ...
                      return (
                        <div key={idx} className="bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all">
                          ...
                          <div 
                            className={`p-5 flex flex-col md:flex-row justify-between items-start md:items-center bg-white border-l-4 border-l-blue-500 ${costSubTab !== 'monthly' ? 'cursor-pointer hover:bg-slate-50 transition' : ''}`}
                            onClick={() => costSubTab !== 'monthly' ? toggleProjectExpand(project.name) : undefined}
                          >
                            <div className="text-right mb-4 md:mb-0 flex items-center gap-4">
                              {costSubTab !== 'monthly' && (
                                <div className={`p-1.5 rounded-full bg-slate-100 text-slate-500 transition-transform ${isExpanded ? 'rotate-180 bg-blue-100 text-blue-600' : ''}`}>
                            ...
                            <div className="bg-slate-900 text-white px-5 py-2.5 rounded text-lg font-bold shadow-sm tracking-wide">
                                ...
                                <div className="flex flex-wrap gap-3">
                                  {(() => {
                                      ...
                                      return (
                                        <>
                                          {hasApt && (
                                            <span className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded border border-indigo-100 text-sm font-medium shadow-sm">
                                              <Building2 className="w-4 h-4" /> כמות דירות: <strong className="text-indigo-900">{aptVal}</strong>
                                            </span>
                                          )}
                                          {s.has_sub_stages ? (
                                            <>
                                              {hasPTyp && (
                                                <span className="flex items-center gap-1.5 bg-blue-50 text-blue-700 px-3 py-1.5 rounded border border-blue-100 text-sm font-medium shadow-sm">
                                                  <Layers className="w-4 h-4" /> טיפוסי אב: <strong className="text-blue-900">{pTypVal}</strong>
                                                </span>
                                              )}
                                              {hasSTyp && (
                                                <span className="flex items-center gap-1.5 bg-blue-50 text-blue-700 px-3 py-1.5 rounded border border-blue-100 text-sm font-medium shadow-sm">
                                                  <Layers className="w-4 h-4" /> תתי-טיפוס: <strong className="text-blue-900">{sTypVal}</strong>
                                                </span>
                                              )}
                                              {vdcCostPerParentTypology !== null && costSubTab !== 'monthly' && (
                                                <span className="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded border border-emerald-100 text-sm font-medium shadow-sm">
                                                  <Wallet className="w-4 h-4" /> עלות VDC לטיפוס אב: <strong className="text-emerald-900">₪ {Math.round(vdcCostPerParentTypology).toLocaleString()}</strong>
                                                </span>
                                              )}
                                              {vdcCostPerSubTypology !== null && costSubTab !== 'monthly' && (
                                                <span className="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded border border-emerald-100 text-sm font-medium shadow-sm">
                                                  <Wallet className="w-4 h-4" /> עלות VDC לתת-טיפוס: <strong className="text-emerald-900">₪ {Math.round(vdcCostPerSubTypology).toLocaleString()}</strong>
                                                </span>
                                              )}
                                            </>
                                          ) : (
                                            <>
                                              {hasTyp && (
                                                <span className="flex items-center gap-1.5 bg-blue-50 text-blue-700 px-3 py-1.5 rounded border border-blue-100 text-sm font-medium shadow-sm">
                                                  <Layers className="w-4 h-4" /> כמות טיפוסים: <strong className="text-blue-900">{typVal}</strong>
                                                </span>
                                              )}
                                              {vdcCostPerTypology !== null && costSubTab !== 'monthly' && (
                                                <span className="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded border border-emerald-100 text-sm font-medium shadow-sm">
                                                  <Wallet className="w-4 h-4" /> עלות VDC לטיפוס: <strong className="text-emerald-900">₪ {Math.round(vdcCostPerTypology).toLocaleString()}</strong>
                                                </span>
                                              )}
                                            </>
                                          )}
                                          {costPerApartment !== null && costSubTab !== 'monthly' && (
                                            <span className="flex items-center gap-1.5 bg-purple-50 text-purple-700 px-3 py-1.5 rounded border border-purple-100 text-sm font-medium shadow-sm">
                                              <Wallet className="w-4 h-4" /> עלות VDC לדירה: <strong className="text-purple-900">₪ {Math.round(costPerApartment).toLocaleString()}</strong>
                                            </span>
                                          )}
                                        </>
                                      );
                                  ...
                                  <div key={sIdx} className="bg-white border border-slate-200 rounded-md shadow-sm flex flex-col overflow-hidden transition-all hover:shadow-md">
                                    <div className={`p-4 border-b ${stg.subStages.length > 0 ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-800'}`}>
                                      ...
                                      <div className="flex justify-between items-center text-sm">
                                        <span className="opacity-80">{stg.days} ימ׳ עבודה</span>
                                        <span className="font-black text-lg">₪ {Math.round(stg.cost).toLocaleString()}</span>
                                      </div>
                                    </div>
                                        ...
                                        {stg.subStages.map((sub: any, subIdx: number) => (
                                          <div key={`${sIdx}-${subIdx}`} className="flex justify-between items-center p-2.5 bg-white rounded border border-slate-200 shadow-sm text-sm">
                                            <div className="flex items-center gap-2 flex-1 truncate">
                                              ...
                                              <span className="text-slate-700 font-medium truncate" title={sub.name}>{sub.name}</span>
                                            </div>
                                            <div className="flex flex-col items-end w-20 flex-shrink-0">
                                              <span className="font-black text-blue-700 text-[13px]">₪ {Math.round(sub.cost).toLocaleString()}</span>
                                              <span className="text-slate-500 text-[11px]">{sub.days} ימ׳</span>
                                            </div>
                                          </div>
                                        ))}
                                      </div>
                    ...
                    return (
                      <>
                        {gen2Projects.length > 0 && (
                          <div className="mb-8">
                            <h3 className="text-lg font-bold text-blue-700 mb-4 flex items-center gap-2 border-b-2 border-blue-100 pb-2">
                              <Layers className="w-5 h-5" /> פרויקטים בתמחור מתקדם (דור 2)
                            </h3>
                            <div className="flex flex-col gap-4">
                              {gen2Projects.map((project, idx) => renderProjectCard(project, idx))}
                            </div>
                          </div>
                        )}
                        ...
                        {gen1Projects.length > 0 && (
                          <div>
                            <h3 className="text-lg font-bold text-slate-600 mb-4 flex items-center gap-2 border-b-2 border-slate-200 pb-2">
                              <Building2 className="w-5 h-5" /> פרויקטים בתמחור בסיסי (דור 1)
                            </h3>
                            <div className="flex flex-col gap-4">
                              {gen1Projects.map((project, idx) => renderProjectCard(project, idx))}
                            </div>
                          </div>
                        )}

                        {displayedCostData.length === 0 && (
                          <div className="col-span-full p-12 text-center text-slate-500 bg-white rounded-md border border-slate-200 flex flex-col items-center gap-3">
                            <Info className="w-8 h-8 text-slate-300" /> אין נתונים להצגה התואמים לסינון הנבחר.
                          </div>
                        )}
                      </>
                    );
          ...
          {editingReport && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-md shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 text-slate-800">
                <div className="bg-slate-800 p-4 border-b border-slate-700 flex justify-between items-center">
                  <h2 className="text-lg font-bold text-white flex items-center gap-2"><Edit2 className="w-4 h-4 text-blue-400" /> עריכת רשומת דיווח</h2>
                  <button onClick={() => setEditingReport(null)} className="text-slate-400 hover:text-white transition"><X className="w-5 h-5" /></button>
                </div>
                <form onSubmit={handleUpdateSubmit} className="p-6 space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">תאריך</label>
                    <input type="date" required value={editDate} onChange={(e) => setEditDate(e.target.value)} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">מהנדס</label>
                    <select required value={editEngineerName} onChange={(e) => setEditEngineerName(e.target.value)} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition bg-white">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">פרויקט</label>
                    <select required value={editProject} onChange={(e) => {
                        const newProject = e.target.value;
                        ...
                        if (newProject.includes('תקורות חברה')) {
                          const hachsharaExists = stagesList.find(s => s.stage_name === 'הכשרה');
                          if (hachsharaExists) setEditStage('הכשרה');
                        }
                      }} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition bg-white">
                  ...
                  <div>
                    ...
                    <select required value={editStage} onChange={(e) => {
                        setEditStage(e.target.value);
                        setEditSubStage('');
                      }} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition bg-white">
                      <option value="" disabled>בחר שלב הנדסי...</option>
                      {availableEditStages.map((s) => (
                        <option key={s.id} value={s.stage_name}>{s.stage_name}</option>
                      ))}
                      {editStage && !availableEditStages.find(s => s.stage_name === editStage) && (
                        <option value={editStage}>{editStage} (היסטורי)</option>
                      )}
                    </select>
                  </div>
                  ...
                  {isEditSubStageRequired && (
                     <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                       ...
                       <select required value={editSubStage} onChange={(e) => setSubStage(e.target.value)} className="w-full rounded-md border-blue-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition bg-blue-50/30 text-blue-900">
                         <option value="" disabled>בחר תת-שלב...</option>
                         {availableEditSubStages.map((s) => (
                           <option key={s.id} value={s.sub_stage_name}>{s.sub_stage_name}</option>
                         ))}
                         {editSubStage && !availableEditSubStages.find(s => s.sub_stage_name === editSubStage) && (
                           <option value={editSubStage}>{editSubStage} (היסטורי)</option>
                         )}
                       </select>
                     </div>
                  )}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">היקף משרה</label>
                    <select value={editScope} onChange={(e) => setEditScope(e.target.value)} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition bg-white">
                  ...
                  <div>
                    ...
                    <textarea value={editNotes} onChange={(e) => setEditNotes(e.target.value)} rows={2} className="w-full rounded-md border-slate-300 p-2.5 text-sm border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition" />
                  </div>
                  <div className="flex justify-end gap-3 pt-5 border-t border-slate-100 mt-2">
                    <button type="button" onClick={() => setEditingReport(null)} className="px-5 py-2.5 bg-white border border-slate-300 text-slate-700 rounded-md font-medium hover:bg-slate-50 transition text-sm">ביטול</button>
                    <button type="submit" disabled={editLoading} className="px-5 py-2.5 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-700 transition shadow-sm text-sm">עדכן רשומה</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          {/* ========================================================================= */}
          {/* מודל מרכזי (Central Modal): תיק מהנדס ופגישות סטאטוס */}
          {/* ========================================================================= */}
          {isDrawerOpen && (
              ...
              <div className="bg-slate-50 w-full max-w-6xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-slate-900 p-5 flex justify-between items-center shadow-md shrink-0">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-white font-black text-xl tracking-tight">תיק מהנדס ופגישות עבודה</h2>
                        <div className="text-slate-400 text-sm font-medium mt-0.5">מהנדס: <span className="text-blue-300">{drawerEngineer}</span></div>
                      </div>
                   </div>
                   <button onClick={closeEngineerDrawer} className="text-slate-400 hover:text-white bg-slate-800 hover:bg-rose-500/20 hover:text-rose-400 p-2.5 rounded-full transition-colors">
                     <X className="w-5 h-5" />
                   </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-8 scrollbar-thin">
                  ...
                  {!drawerProject ? (
                    <div className="animate-in fade-in duration-500">
                      <h3 className="font-bold text-slate-700 mb-6 flex items-center gap-2 text-base uppercase tracking-wider">
                        <Building2 className="w-5 h-5 text-blue-500" /> בחר פרויקט לניהול פגישה
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {getProjectsForEngineer(drawerEngineer!).map((proj, idx) => (
                          <button 
                            key={idx} 
                            onClick={() => selectProjectForMeeting(proj)}
                            className="w-full text-right p-5 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-lg hover:border-blue-400 transition-all flex justify-between items-center group"
                          >
                            <span className="font-bold text-slate-800 text-base group-hover:text-blue-700 transition">{proj}</span>
                            <ArrowRight className="w-5 h-5 text-slate-300 group-hover:text-blue-500 transition-transform group-hover:-translate-x-1.5" />
                          </button>
                        ))}
                        {getProjectsForEngineer(drawerEngineer!).length === 0 && (
                          <div className="col-span-full text-slate-400 text-base text-center p-12 bg-white border border-slate-200 rounded-xl border-dashed">לא נמצאו פרויקטים פעילים למשתמש זה.</div>
                        )}
                      </div>
                    </div>
                  ) : (
                    /* שלב 2: ניהול פגישות לפרויקט נבחר */
                    <div className="animate-in fade-in duration-500 h-full flex flex-col">
                      
                      <div className="flex justify-between items-end mb-6 border-b-2 border-slate-200 pb-4">
                        <div>
                          <div className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-1">פרויקט נבחר</div>
                          <div className="text-2xl font-black text-slate-800">{drawerProject}</div>
                        </div>
                        <button 
                          onClick={() => { setDrawerProject(null); setIsNewMeetingOpen(false); setEditingMeeting(null); }}
                          className="flex items-center gap-1.5 text-sm font-bold text-slate-500 hover:text-slate-800 transition bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm hover:bg-slate-100"
                        >
                           <ArrowRight className="w-4 h-4" /> חזור לפרויקטים
                        </button>
                      </div>

                      {/* פריסת 2 עמודות - טופס ימין, הקשר משמאל */}
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        
                        {/* עמודה ימנית: טופס פגישה או לחצן פתיחה + היסטוריה */}
                        <div className="lg:col-span-8 flex flex-col gap-8">
                          ...
                          {isNewMeetingOpen ? (
                            <div className="bg-white rounded-xl shadow-lg border border-blue-200 overflow-hidden animate-in zoom-in-95 duration-200">
                               <div className="bg-blue-600 text-white p-4 font-bold flex items-center gap-2 text-lg">
                                 <PenTool className="w-5 h-5" /> {editingMeeting ? 'עדכון סיכום סטטוס שבועי' : 'עריכת סיכום סטטוס שבועי'}
                               </div>
                               <form onSubmit={handleMeetingSubmit} className="p-6 md:p-8 space-y-6">
                                  <div className="w-1/3">
                                    <label className="block text-sm font-bold text-slate-600 mb-1.5">תאריך הפגישה</label>
                                    <input type="date" required value={meetingDate} onChange={(e) => setMeetingDate(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-md text-base text-slate-800 font-medium outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all bg-white" />
                                  </div>
                                  <div>
                                    <label className="block text-sm font-bold text-slate-600 mb-1.5">1. סטטוס התקדמות לפי מלאכות</label>
                                    <textarea required value={meetingProgress} onChange={(e) => setMeetingProgress(e.target.value)} rows={5} className="w-full p-3 border border-slate-300 rounded-md text-base text-slate-800 font-medium leading-relaxed outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 bg-blue-50/30 transition-all" />
                                  </div>
                                  <div>
                                    <label className="block text-sm font-bold text-slate-600 mb-1.5">2. חסמים מרכזיים</label>
                                    <textarea value={meetingBottlenecks} onChange={(e) => setMeetingBottlenecks(e.target.value)} rows={4} className="w-full p-3 border border-slate-300 rounded-md text-base text-slate-800 font-medium leading-relaxed outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 bg-rose-50/30 transition-all" />
                                  </div>
                                  <div>
                                    <label className="block text-sm font-bold text-slate-600 mb-1.5">3. מיקוד לשבוע הקרוב</label>
                                    <textarea required value={meetingFocus} onChange={(e) => setMeetingFocus(e.target.value)} rows={4} className="w-full p-3 border border-slate-300 rounded-md text-base text-slate-800 font-medium leading-relaxed outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 bg-emerald-50/30 transition-all" />
                                  </div>
                                  <div>
                                    <label className="block text-sm font-bold text-slate-600 mb-1.5">4. מעקב ממדלים (אם יש)</label>
                                    <textarea value={meetingModelers} onChange={(e) => setMeetingModelers(e.target.value)} rows={3} className="w-full p-3 border border-slate-300 rounded-md text-base text-slate-800 font-medium leading-relaxed outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all bg-white" placeholder="אופציונלי..." />
                                  </div>
                                  
                                  <div className="flex justify-end gap-4 pt-6 border-t border-slate-100">
                                    <button type="button" onClick={() => { setIsNewMeetingOpen(false); setEditingMeeting(null); }} className="px-6 py-2.5 bg-slate-100 text-slate-600 rounded-md text-sm font-bold hover:bg-slate-200 transition">ביטול</button>
                                    <button type="submit" disabled={meetingLoading} className="px-8 py-2.5 bg-blue-600 text-white rounded-md text-base font-bold hover:bg-blue-700 transition shadow-md">{editingMeeting ? 'עדכן סיכום פגישה' : 'שמור סיכום פגישה'}</button>
                                  </div>
                               </form>
                            </div>
                          ) : (
                            <button 
                              ...
                              className="w-full py-5 bg-slate-800 text-white rounded-xl font-bold text-lg hover:bg-slate-700 transition shadow-lg flex items-center justify-center gap-3"
                            >
                              <Plus className="w-6 h-6" /> התחל פגישת סטטוס חדשה לפרויקט
                            </button>
                          )}

                          {/* --- היסטוריית פגישות --- */}
                          <div>
                            <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2 text-base uppercase tracking-wider">
                              <FileText className="w-5 h-5 text-amber-500" /> היסטוריית פגישות
                            </h3>
                                 ...
                                 {meetingHistory.map(m => (
                                   <div key={m.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                                     <div className="bg-slate-100/80 border-b border-slate-200 p-4 flex justify-between items-center">
                                       <div className="flex items-center gap-4">
                                          ...
                                          <div className="flex gap-2">
                                            <button onClick={() => openEditMeetingForm(m)} className="p-1.5 bg-white border border-slate-200 rounded text-slate-500 hover:text-blue-600 hover:bg-blue-50 transition" title="ערוך פגישה">
                                              <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDeleteMeeting(m.id)} className="p-1.5 bg-white border border-slate-200 rounded text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition" title="מחק פגישה">
                                     ...
                                     <div className="p-6 space-y-5 text-base">
                                        <div>
                                          <div className="font-bold text-slate-500 text-sm uppercase tracking-wide mb-1.5">סטטוס התקדמות:</div>
                                          <div className="text-slate-800 whitespace-pre-wrap leading-relaxed">{m.progress_status}</div>
                                        </div>
                                        {m.bottlenecks && (
                                          <div className="bg-rose-50/70 p-4 rounded-lg border border-rose-100">
                                            <div className="font-bold text-rose-700 text-sm uppercase tracking-wide mb-1.5">חסמים מרכזיים:</div>
                                            <div className="text-rose-900 whitespace-pre-wrap leading-relaxed">{m.bottlenecks}</div>
                                          </div>
                                        )}
                                        <div>
                                          <div className="font-bold text-slate-500 text-sm uppercase tracking-wide mb-1.5">מיקוד שבועי:</div>
                                          <div className="text-slate-800 whitespace-pre-wrap font-medium leading-relaxed">{m.weekly_focus}</div>
                                        </div>
                                        {m.modelers_tracking && (
                                          <div className="pt-4 border-t border-slate-100">
                                            <div className="font-bold text-indigo-500 text-sm uppercase tracking-wide mb-1.5">מעקב ממדלים:</div>
                                            <div className="text-slate-700 whitespace-pre-wrap leading-relaxed">{m.modelers_tracking}</div>
                                          </div>
                                        )}
                                     </div>
                                   </div>
                        ...
                        {/* עמודה שמאלית: Context דיווחים מהשבוע האחרון */}
                        <div className="lg:col-span-4">
                          <div className="bg-slate-100 rounded-xl p-5 border border-slate-200 h-full max-h-[800px] flex flex-col">
                            <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                              <Clock className="w-4 h-4 text-emerald-500" /> דו"ח פעילות - 7 ימים אחרונים
                            </h3>
                                   ...
                                   {recentReportsContext.map(r => (
                                     <div key={r.id} className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm flex flex-col gap-2 transition hover:border-emerald-300">
                                       ...
                                       <div className="font-bold text-slate-800 text-base">
                                          {r.stage} {r.sub_stage && <span className="text-slate-400 font-normal">| {r.sub_stage}</span>}
                                       </div>
